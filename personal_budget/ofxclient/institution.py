...
from personal_budget.ofxclient.client import Client
...
class Institution(object):
    ...
    def __init__(self, id, org, url, username, password,
        ...
        self.use_in_mem_string_buffer = use_in_mem_string_buffer
    ...
    def accounts(self):
        ...
        """
        from personal_budget.ofxclient.account import Account
        client = self.client()
        query = client.account_list_query()
        resp = client.post(query)
        if self.use_in_mem_string_buffer:
            resp_handle = StringIO(resp)
        else:
            resp_handle = open('/tmp/ofxclient.ofx', 'w+')
            resp_handle.write(resp)

        parsed = OfxParser.parse(resp_handle)
        if not self.use_in_mem_string_buffer:
            resp_handle.close()
